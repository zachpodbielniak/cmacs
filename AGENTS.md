...
**`gowl-stop` and what the compositor owns.** gowl's compositor only *borrows* its config and module manager, so cmacs hands its references to the compositor as object data on both launch paths (`cmacs_gowl_hand_over_config_and_modules`, called by `Fgowl_start` and the `--gowl` branch of `main()`); they are released at the end of the compositor's finalization, in gowl's `main()` order. Replacing the config (`gowl-reload-config` with no file) goes through `cmacs_gowl_replace_config`, which swaps under the gowl lock and releases the old one; gowl's own `reload_config` keybind releases only configs the compositor made itself. Never free them separately in `gowl-stop`: `gowl-compositor` gives Lisp a strong reference, and an uncollected wrapper keeps the compositor alive past `gowl-stop`, to be finalized later against whatever was freed. `gowl-stop` dispatches every module's shutdown hook while the compositor is alive, then resets cmacs's per-compositor statics in `cmacs_gowl_forget_compositor_state` — add any new ones there, or the next `gowl-start` inherits them (stale clipboard handler ids made `gowl-clipboard-watch` connect nothing). `test/cmacs/cmacs-gowl-tests.el` runs real headless start/stop cycles in a child cmacs.

**The gowl lock, and which thread you are on.** The dispatch thread holds the recursive `cmacs_gowl_mutex` across every `wl_event_loop_dispatch`, so anything running on Emacs's thread that touches compositor state has to take that same lock. The config matters most: it is not merely mutated but *replaced* — by `gowl-reload-config` with no file, and by gowl's own reload keybind — so an unlocked read can land on a config that is being released underneath it. In a DEFUN use `cmacs_gowl_lock_scoped()`, which takes the lock and records the release on the specpdl; these bodies signal (`error` for no config, an unparseable key, an unknown property) and `error` longjmps straight past any unlock written after the body, leaving the mutex held and wedging the dispatch thread — a frozen desktop, strictly worse than the race. Every exit then returns through `unbind_to (count, ...)`. The RPC functions in `cmacs/glib/cmacs-eval-dispatch.c` (D-Bus, MCP, bacon, podomation) run on the same thread and take the same lock, with a plain unlock on each path because nothing there signals -- `cmacs-gowl-test-dispatch-functions-take-the-lock` refuses a `cmacs_dispatch_gowl_*` body that calls into gowl without one, and `-dispatch-lock-and-unlock-balance` refuses a `return` between a lock and its unlock (this paragraph said they took it before they all did; the guard is what makes it true). `cmacs-gowl-test-config-defuns-take-the-lock`, `-scoped-lock-releases-through-specpdl` and `-scoped-lock-returns-unwind` fail the suite if a new call site skips either half. It is not only the config: since 2026-09 **every DEFUN that reaches the compositor** -- its client and focus lists (`gowl_compositor_get_clients` returns the live list), the seat, the scene, the module set -- takes `cmacs_gowl_lock_scoped()` first, and `cmacs-gowl-test-compositor-defuns-take-the-lock` refuses a new one that does not; sixty of them ran unlocked before. The two exceptions have reasons in the source: `gowl-stop` joins the dispatch thread (a lock across the join is a deadlock), and `gowl-clipboard-get` / `gowl-primary-selection-get` lock the *ask* (`gowl_seat_open_clipboard`, which also flushes) and not the *wait* (`gowl_seat_read_selection_fd`), because the owning client can only answer while the dispatch thread is free. Every cmacs path that moves seat focus behind the compositor's back asks `gowl_compositor_keyboard_is_grabbed()` -- both grabs the focus gate refuses for, a launcher's and an X11 popup's -- never the layer-only query.
...
**Gate tests on the primitive, not on a feature name that does not exist.**
`cmacs-feature-p` (cmacs.el) knows the configure-time names from
`cmacs_feature_names[]`; `gobject` is not one (the bridge ships with `glib`), so
`(skip-unless (cmacs-feature-p 'gobject))` skipped every GObject test in every
run for months and the suite proved nothing. Use `(fboundp 'gobject-new)` and
friends. A suite whose log says "N skipped" for a subsystem that is built has
this bug.

**`libcmacs-api.so` and the bacon modules are not rebuilt by a top-level make.**
`src/Makefile.in` only builds them when the file is *absent*, so a change under
`cmacs/api/` or `cmacs/bacon/modules/` is invisible to `bacon-eval "cmacsgi …"`
and to crispy scripts until you run `make -C cmacs/api all` and
`make -C cmacs/bacon/modules BACON_DIR=$PWD/deps/bacon API_DIR=$PWD/cmacs/api`.
The symptom is a test that keeps failing against a fix you can read in the
source.
