...
./configure \             # see README.org for full recommended flags
  ...
  --with-cmacs-print \
  --with-cmacs-video \
  --with-cmacs-cintrospect --with-cmacs-libregnum \
  --with-cmacs-gsurf \
  --enable-cmacs-cpatch
...
| **gsurf** | `cmacs/gsurf/` + `lisp/cmacs/cmacs-gsurf.el` | Embedded web browser — gsurf (GLib/GObject suckless-`surf` port) as live WebKitGTK buffers. `cmacs-gsurf-mode` parents a `GsurfView`'s native widget into the pgtk frame's `GtkFixed` (`FRAME_GTK_WIDGET`, xwidget-style live embed) and the Elisp `window-configuration-change-hook`/`window-size-change-functions` reposition it via `cmacs-gsurf-place` (hide off-screen via `cmacs-gsurf-hide`). **Focus handoff** (critical — a live GTK child otherwise steals all keyboard focus and Emacs/evil never see keys, because a page that autofocuses an element grabs the GtkWindow's keyboard focus away from the Emacs edit widget): the web widget is created **`gtk_widget_set_can_focus(widget, FALSE)`** so it can NEVER become the toplevel's focus widget — `gtk_widget_grab_focus` is a no-op on a non-focusable widget, so WebKit autofocus (DuckDuckGo's search box, etc.) cannot steal keys and Emacs keeps every key (`SPC` leader, `C-w`, `M-x`) by default. Pointer events (clicks, wheel, link nav) are delivered regardless of focusability, so mouse browsing works fully. Keyboard is handed to the page ONLY by an explicit `cmacs-gsurf-focus-page` (`RET`/`i`) or `cmacs-gsurf-follow` (`f`), which flip `can_focus` TRUE + `grab_focus` + set the per-view `focused` intent flag. Escape is intercepted in `on_view_key_press` (dispatched to modal for cleanup, then **unconditionally** `release_to_emacs` — sets `can_focus` FALSE + grabs the frame + `(cmacs-gsurf--on-escape)` to drop evil to normal); `window-selection-change-functions` → `cmacs-gsurf--update-focus` → `cmacs-gsurf-release-focus` (which makes EVERY view non-focusable + grabs the frame) restores keys when a non-gsurf window is selected. `on_view_key_press` also has a safety net: a key arriving while `!focused` bounces focus back to Emacs and is swallowed. `cmacs-gsurf-page-focused-p` returns the `focused` intent flag (NOT `gtk_widget_has_focus`, which lies for WebKit). Evil: initial state `normal`, browser keys bound via `evil-define-key*` in normal+motion so `C-w`/motions stay with evil. DEFUNs `cmacs-gsurf-focus-page`/`-release-focus`/`-page-focused-p`/`-follow`. **Key dispatch** (so the `modal` module's `hjkl`/`f`/`i` work — the raw embed has no `GsurfWindow`, so nothing called the module manager): `on_view_key_press` routes page keystrokes through `gsurf_module_manager_dispatch_key_event` (passthrough-guarded by `get_input_passthrough`/`gsurf_view_get_editing`). **Escape is checked FIRST and ALWAYS returns focus to Emacs** — it is dispatched to the modules for cleanup (clear hint overlay / leave INSERT) but the handler IGNORES whether modal consumed it and unconditionally calls `release_to_emacs` + `(cmacs-gsurf--on-escape)`. (Earlier bug: dispatch-then-return let the modal module swallow Escape — it consumes Escape in both NORMAL and INSERT — so focus stayed on the WebKit page and Space kept scrolling the page even after Esc. `SPC` itself is correctly unbound on the Emacs side; the symptom was a focus-retention bug, not a keymap bug.) The `modal` module (enabled by default, `scroll_step=60`, `hint_chars=asdfghjkl`) provides `hjkl` scroll + `f` link hints + `i` insert once the page is focused. Emacs-side `h/j/k/l` (+`C-d/C-u` half-page, `C-f/C-b` full-page, `gg/G` top/bottom) also scroll via `cmacs-gsurf-run-javascript` (`window.scrollBy`) **without** focusing the page (buffer text is a hidden placeholder, so these rebind evil motions); `f` → `cmacs-gsurf-follow` focuses the page and dispatches `f` to pop hints. `SPC` is deliberately **not** bound so the evil/Doom leader (`SPC b b`, …) passes through. gsurf.h is GTK-free, so only `cmacs-gsurf-view.c` pulls in gtk/pgtkterm; the defun layer stays Lisp-only. Built `STATIC=1 BUILD_GIR=1 GTK_BACKEND=gtk3`; `libgsurf.a` (which merges crispy+yaml-glib via `ar -M`) is whole-archived into temacs and **dedup'd** (`_gsurf_strip` strips `$(_dup_crispy)`+`$(_dup_yaml)`), never `-z muldefs`. temacs is `--export-dynamic`, so runtime-loaded gsurf module `.so`s resolve `gsurf_*`/`yaml_*` from the binary (gsurf's own `STATIC=1`/`LIBGSURF_LINK=` contract). View `uri/title/load` signals bridge to Emacs abnormal hooks (`cmacs-gsurf-{uri,title,load}-changed-functions`). Drive it from Elisp (`cmacs-gsurf`, `cmacs-gsurf-browse-url`), cmacsgi (`init_cmacs_gsurf` prepends the bundled `Gsurf-0.1` typelib to the GI search path → `(gi-require "Gsurf" "0.1")`), MCP (`gsurf_open`/`gsurf_navigate`/`gsurf_current`/…), and cmacs-ai (`^gsurf_` in the default allowlist). gsurf's 29 stock modules (search engines, ad-block, vim modal + link hints, history, dark mode, …) load from the search path; custom cmacs modules live in `cmacs/gsurf/modules/` (dir set via `CMACS_GSURF_MODULE_DIR`, exported by `just run`, like bacon modules). **Config is Emacs-driven, NOT from `~/.config/gsurf/config.yaml`** (gsurf modules default to `enabled=FALSE`, so without config the `modal` module — and thus `f`/in-page `hjkl` — is never active). Bring-up is split: `cmacs_gsurf_config_ensure` creates the display-free `GsurfConfig` (built-in defaults only) so Emacs can load into it BEFORE `cmacs_gsurf_modules_init` loads/activates modules at first attach. The Elisp `cmacs-gsurf-modules` defcustom (alist `(NAME . PLIST)`, `:enabled`+options; default enables `modal`/`search_engines`/`history`) is synthesized to a `modules:` YAML by `cmacs-gsurf--config-to-yaml` and loaded via `gsurf_config_load_from_data` (gsurf modules read their config from the `modules:` YAML mapping — there is **no** typed C setter for module enable/options, despite what a commit subject might imply; the only real config setters added upstream are `gsurf_config_set_keybind`/`_set_mousebind`, plus the `ignore_yaml` YAML root key). Applied by `cmacs-gsurf--apply-config` in `cmacs-gsurf-mode` init, before attach. Opt-in to gsurf's own files via `cmacs-gsurf-load-user-config`/`-config-file`/`-config-c-file` (the C-file path uses gsurf's crispy `GsurfConfigCompiler`, e.g. `~/.config/cmacs/init.c`, whose `gsurf_config_init()` may call any `gsurf_config_*` setter incl. `gsurf_config_set_keybind`). DEFUNs: `cmacs-gsurf-load-config-data`/`-file`/`-c-file`, `cmacs-gsurf-reconfigure-modules`; `M-x cmacs-gsurf-reload-config`. (gsurf's *library* never reads `~/.config/gsurf/config.yaml` itself — that's its CLI binary's policy — so the embed simply never loads it unless asked.) Off by default — `--with-cmacs-gsurf` (alias `--with-gsurf`); requires `--with-cmacs-glib` + `--with-pgtk`. |
...
- **gsurf**: top-level `deps/gsurf` submodule (a GLib/GObject port of suckless `surf`), built once at `deps/gsurf/build/release/` with `make STATIC=1 BUILD_GIR=1 GTK_BACKEND=gtk3` (driven by the `$(CMACS_GSURF_STATIC_LIB)` rule in `src/Makefile.in`). Requires `--with-cmacs-glib` and `--with-pgtk`. PKG_CHECK_MODULES for `glib-2.0 gobject-2.0 gio-2.0 gmodule-2.0 yaml-0.1 json-glib-1.0 webkit2gtk-4.1 gtk+-3.0 javascriptcoregtk-4.1` — the same WebKitGTK/GTK3 stack as `--with-xwidgets`, so no new packages. `libgsurf.a` is `-Wl,--whole-archive`d into temacs; because it merges crispy + yaml-glib objects it is run through the dedup machinery (`_gsurf_strip` → `libgsurf-dedup.a`, stripping `$(_dup_crispy)` + `$(_dup_yaml)`), **never** `-z muldefs`. temacs is linked `--export-dynamic` so gsurf modules loaded at runtime resolve `gsurf_*`/`yaml_*` against the executable (one library instance — gsurf's `STATIC=1` host-embed contract). `init_cmacs_gsurf` prepends `CMACS_GSURF_GIR_DIR` (the bundled `Gsurf-0.1.typelib`) to the GIRepository search path, so `(gi-require "Gsurf" "0.1")` and the bacon `cmacsgi` builtin work with no `GI_TYPELIB_PATH`. Module search paths (first match by filename wins): `$CMACS_GSURF_MODULE_DIR` (set by `just run`) → `$GSURF_MODULE_PATH` → `CMACS_GSURF_MODULE_DIR` (`cmacs/gsurf/modules`) → `CMACS_GSURF_DEV_MODULE_DIR` (`deps/gsurf/build/release/modules`, the stock 29) → `GSURF_MODULEDIR` (`$libdir/cmacs/gsurf/modules`). MCP tools `gsurf_*` (gated on `--with-cmacs-mcp`); `^gsurf_` is in the default `cmacs-ai-mcp-bridge-allowlist`. Defcustoms: `cmacs-gsurf-home-url`, `cmacs-gsurf-default-zoom`, `cmacs-gsurf-search-url`. Alias flag `--with-gsurf`.
...
- **Agent Bash tool: `pkill`/`grep` exit-1 cancels the whole batch.** The session shell runs with `set -e`-style behavior, so any command that exits non-zero on a *non-error* (e.g. `pkill -f 'make -j'` when nothing matches → exit 1; `grep` with no match → exit 1) aborts the script AND, in a parallel tool-call batch, cancels every sibling call. Defenses: append `|| true` to such commands (`pkill -9 -f 'make -j' 2>/dev/null || true`), end build/process scripts with `exit 0`, and run build/kill/verify steps **one Bash call per message** (not batched) so an exit-1 can't nuke siblings. Local build/process steps also need `dangerouslyDisableSandbox: true` (the sandbox blocks process kills).
