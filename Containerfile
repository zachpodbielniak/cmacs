...
#
# Order matters, and now for two reasons.  ai-glib MUST come before
# podomation and libreclaw: both consume it through AI_GLIB_DIR, and
# with the build phase running -j a consumer must never be the thing
# that first produces it -- two sub-makes into one tree is the `ar'
# race that src/Makefile.in's cmacs-canonical-deps target exists to
# prevent.  ai-glib itself is built with GIR=1 so AiGlib-1.0.typelib
# lands in the system GI search path for downstream consumers
# (python-gi, gjs, the bacon `cmacsgi' builtin, etc.).
#
...
# that bundles crispy / yaml-glib / mcp-glib / podomation is pointed at
# the canonical checkout.  Without it these sub-makes reach for bundled
# copies that admin/cmacs-submodules.sh deliberately does not clone, and
# the build fails on a directory that is empty on purpose.  Both
# spellings are passed because the deps disagree (YAMLGLIB_DIR in gowl
# and bacon, YAML_GLIB_DIR in podomation, libreclaw and ai-glib); a make
# variable a given dep does not use is simply ignored.  PODOMATION_DIR
# was the one that got missed, and it cost the most: libreclaw went on
# building its own deps/podomation checkout -- the library AND all forty
# of its modules -- a second time, every image.
#
# Three phases rather than one `make clean all', because the build phase
# runs -j now and `make -j clean all' would run the clean CONCURRENTLY
# with the build it is supposed to precede:
#   1. clean every dep, serially;
#   2. build the shared archives -- crispy, yaml-glib, mcp-glib -- once
#      and serially, so that every consumer's sub-make into them is a
#      no-op.  This is the container's copy of cmacs-canonical-deps.
#      yaml-glib is here despite not being in the dep list below because
#      nothing else builds it first: it used to appear as a side effect
#      of whichever consumer reached it first, which under -j is two of
#      them at once.  Built, deliberately NOT installed -- yaml-glib is
#      only ever vendored statically, never found with pkg-config, and
#      installing it would let a later configure pick it up as a system
#      library instead;
#   3. build each dep under -j, then install it.  install stays serial:
#      it writes into a shared /usr.
#
# PREFIX=/usr goes on EVERY phase, not just the two that used to carry
# it.  A dep's .pc file is generated by the build, not by install, and
# make will not regenerate one that is already up to date -- so building
# a dep once without PREFIX and once with it installs the FIRST spelling.
# That put `prefix=/usr/local' into /usr/lib64/pkgconfig/crispy.pc; every
# later pkg-config consumer then reported crispy as present and handed
# out -I/usr/local/include/crispy, and cad-glib failed on <crispy.h>
# thousands of lines later with nothing pointing at the cause.
#
...
RUN CANON_DIRS="CRISPY_DIR=/build/cmacs/deps/crispy \
      ...
      PODOMATION_DIR=/build/cmacs/deps/podomation" \
    && CMACS_DEPS="deps/mcp-glib deps/crispy deps/bacon deps/gowl \
                   deps/ai-glib deps/podomation \
                   deps/clawtilla/deps/libreclaw" \
    && rm -f .git \
    && find . \( -name '*.elc' -o -name '*.eln' \) -delete \
    && rm -rf native-lisp src/*.pdmp deps/whisper.cpp/build \
    && for dep in deps/yaml-glib ${CMACS_DEPS}; do \
           [ -d "${dep}" ] \
               || { echo "FAILED: ${dep} is not checked out" >&2; exit 1; }; \
       done \
    && for dep in ${CMACS_DEPS}; do \
           make -C "${dep}" clean PREFIX=/usr ${CANON_DIRS} \
               || { echo "FAILED: clean ${dep}" >&2; exit 1; }; \
       done \
    && make -C deps/crispy lib PREFIX=/usr ${CANON_DIRS} \
           || { echo "FAILED: canonical crispy" >&2; exit 1; } \
    && make -C deps/yaml-glib clean DEBUG=0 PREFIX=/usr ${CANON_DIRS} \
           || { echo "FAILED: clean canonical yaml-glib" >&2; exit 1; } \
    && make -C deps/yaml-glib lib DEBUG=0 PREFIX=/usr ${CANON_DIRS} \
           || { echo "FAILED: canonical yaml-glib" >&2; exit 1; } \
    && make -C deps/mcp-glib PREFIX=/usr ${CANON_DIRS} \
           || { echo "FAILED: canonical mcp-glib" >&2; exit 1; } \
    && for dep in ${CMACS_DEPS}; do \
           gir=""; \
           case "${dep}" in */ai-glib) gir="GIR=1";; esac; \
           make -C "${dep}" -j"${CMACS_JOBS:-$(nproc)}" all \
                PREFIX=/usr ${gir} ${CANON_DIRS} \
