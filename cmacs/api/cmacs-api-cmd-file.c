...
gint
cmd_file_open(CmacsApiTransport *transport, gint argc, gchar **argv)
{
    gchar *escaped, *elisp;
    gint rc;
...
gint
cmd_file_save(CmacsApiTransport *transport, gint argc, gchar **argv)
{
    gchar *escaped, *elisp;
    gint rc;
...
gint
cmd_file_close(CmacsApiTransport *transport, gint argc, gchar **argv)
{
    gchar *escaped, *elisp;
    gint rc;

    if (argc < 3)
        return cmacs_api_eval_quiet(transport, "(kill-buffer (current-buffer))");
...
gint
cmd_file_recent(CmacsApiTransport *transport, gint argc, gchar **argv)
{
    gint n, i;
    gchar *elisp;
    gint rc;
