...
static gint
vc_status(CmacsApiTransport *transport, gint argc, gchar **argv)
{
    (void)argc;
    (void)argv;
...
static gint
vc_diff(CmacsApiTransport *transport, gint argc, gchar **argv)
{
    gchar *escaped, *elisp;
    gint rc;
...
static gint
vc_log(CmacsApiTransport *transport, gint argc, gchar **argv)
{
    gint n, i;
    const gchar *file;
    gchar *elisp;
    ...
    if (file != NULL)
    {
        gchar *esc = cmacs_api_lisp_escape(file);
        elisp = g_strdup_printf(
            "(with-temp-buffer"
            " (call-process \"git\" nil t nil"
            "   \"log\" \"--oneline\" \"-n\" \"%d\" \"--\" \"%s\")"
...
static gint
vc_blame(CmacsApiTransport *transport, gint argc, gchar **argv)
{
    gchar *escaped, *elisp;
    gint rc;
    const gchar *file;
    ...
    if (file != NULL)
    {
        escaped = cmacs_api_lisp_escape(file);
        elisp = g_strdup_printf(
            "(with-temp-buffer"
            " (call-process \"git\" nil t nil"
            "   \"blame\" \"--no-color\" \"%s\")"
...
static const CmacsApiSubcmd vc_subcmds[] = {
...
gint
cmd_vc(CmacsApiTransport *transport, gint argc, gchar **argv)
{
    return cmacs_api_dispatch_group("vc", vc_subcmds,
