 ...
 *
 * MCP parity: mirrors ai_prompt / ai_list_providers / ai_open_chat in
 * cmacs/mcp/cmacs-mcp-tools-ai.c (sync discipline: adding a tool there
                ...
                GDBusMethodInvocation *iv, gpointer u)
{
  ...
  if (g_strcmp0 (m, "ListProviders") == 0)
    ...
    {
      const gchar *prompt, *provider, *system;
      gchar *prompt_q, *system_form, *provider_form;
      gchar *expr;
      gchar *result;
      GError *err = NULL;

      g_variant_get (p, "(&s&s&s)", &prompt, &provider, &system);
      ...
      expr = g_strdup_printf (
        ...
        prompt_q, provider_form, system_form);
      g_free (prompt_q);
    ...
    {
      const gchar *provider, *prompt;
      gchar *prompt_q, *provider_form;
      gchar *expr;
      gchar *result;
      GError *err = NULL;

      g_variant_get (p, "(&s&s)", &provider, &prompt);
      if (*provider != '\0' && !valid_provider_name (provider))
        {
          g_dbus_method_invocation_return_dbus_error (
            iv, "org.cmacs.Editor1.Error", "invalid provider name");
      ...
      expr = g_strdup_printf (
        ...
        "   (buffer-name buf)))",
        provider_form, prompt_q, prompt_q, prompt_q);
      g_free (prompt_q);
      g_free (provider_form);
